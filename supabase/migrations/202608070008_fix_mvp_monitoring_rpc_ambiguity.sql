-- Qualify table columns that collide with RETURNS TABLE fields in the MVP
-- monitoring RPCs. This is a forward-only repair for 202608070006.
...
begin
  ...
  update public.radar_runs as run_update
   ...
   where run_update.id = claimed.run_id
     and run_update.status = 'running'
     and run_update.lease_owner = claimed.lease_owner
     and run_update.trigger = 'baseline'
  returning run_update.id into trigger_updated_id;
