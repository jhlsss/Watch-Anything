      ...
      <div className="border-b border-slate-200 bg-[radial-gradient(circle_at_top_right,_rgba(167,139,250,0.18),_transparent_30%),linear-gradient(#ffffff,#faf7ff)]">
        <div className="mx-auto w-full max-w-6xl px-4 sm:px-6">
          <Hero
            ...
            onParseRequest={() => undefined}
          />
        </div>
      </div>

      <section className="bg-[#f5f2fb] py-16">
        <div className="mx-auto grid w-full max-w-6xl gap-6 px-4 sm:px-6 min-[850px]:grid-cols-[0.78fr_1.22fr]">
          ...
          <div className="rounded-3xl border border-violet-100 bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between">
              <p className="font-semibold text-slate-950">LISA Official Radar</p>
              <span className="text-sm font-semibold text-emerald-600">● Running</span>
            </div>
            <p className="mt-2 text-sm text-slate-500">Music releases · Tours · Official partnerships · Reputable sources</p>
        ...
        <div className="mx-auto w-full max-w-6xl px-4 sm:px-6">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            {copy.landing.templates.map(([title, description]) => (
              <article key={title} className="flex items-center justify-between rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-violet-200">
                <div>
