...
async function readRun(
  ...
  const result = await client
    .from("radar_runs")
    .select("id,radar_id,status,trigger,lease_owner,lease_expires_at")
    .eq("id", runId)
    .eq("status", "running")
...
function findExistingByEventKey(
  ...
  return findings.find((finding) => finding.event_key === eventKey);
}
...
async function readExistingFindings(
  ...
  const result = await client
    .from("findings")
    .select(
      "id,radar_id,fingerprint,event_key,first_seen_during_baseline,notification_eligible",
    )
    .eq("radar_id", radarId);
  throwDatabaseError(result.error);
...
async function saveFinding(
  ...
  const existingByEventKey = findExistingByEventKey(existingFindings, eventKey);
  const existing = existingByFingerprint ?? existingByEventKey;
...
export async function executeClaimedRun(
  ...
  const run = await readRun(db, runId, leaseOwner, now);
  const radar = await readRadar(db, run.radar_id);
  ...
  const tavilyFetcher = dependencies.searchTavily ?? (() => searchTavily({ query: radar.rules.searchQuery }));
  ...
  if (uniqueCandidates.length > 0) {
    try {
      ...
      aiFailed = true;
      internalErrors.push({ source: "ai", errorCode: errorCode(error, "AI_EVALUATION_FAILED") });
    }
  }
  ...
  await markSourceBaselineComplete(db, radar, successfulSources, now);
  ...
  if (destinationId) {
    ...
    const sendNotification = dependencies.sendNotification ?? defaultSendPendingNotification;
  ...
  } else if (savedFindings.some((item) => item.eligible && !aiFailed)) {
    telegramFailed = true;
