...
describe("monitoring migration contract", () => {
  ...
  it("qualifies radar_id in requeue_failed_notifications_for_run", () => {
    const migration = readFileSync(
      resolve(
        process.cwd(),
        "supabase/migrations/202608060002_monitoring_functions.sql",
  ...
  it("ships a production correction migration for requeue_failed_notifications_for_run", () => {
    const migration = readFileSync(
      resolve(
        process.cwd(),
        "supabase/migrations/202608070004_fix_requeue_failed_notifications_radar_id.sql",
      ),
      "utf8",
    )
      .replace(/--.*$/gm, "")
    ...
    expect(migration).not.toMatch(/where id = p_run_id and radar_id = radar_id_for_run/);
    expect(migration).toContain(
      "grant execute on function public.requeue_failed_notifications_for_run",
    );
  });
